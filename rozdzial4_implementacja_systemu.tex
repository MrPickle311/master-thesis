...
\begin{itemize}
    \item \textbf{język \texttt{Java 21}} - główny język programowania wykorzystany do implementacji mikroserwisów (np. \texttt{FrontService}, \texttt{DataService}),
    \item \textbf{język \texttt{Scala}} - język programowania wykorzystany do implementacji aplikacji \texttt{SparkDataProcessor} służącej do przetwarzania strumieni danych za pomocą \mbox{Apache Spark},
    \item \textbf{język \texttt{Python 3.9}} - język wykorzystany do implementacji symulatorów czujników i skryptów pomocniczych (np. do trenowania modelu uczenia maszynowego),
    \item \textbf{język \texttt{TypeScript}} - język wykorzystany do implementacji interfejsu użytkownika.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{silnik analityczny \mbox{Apache Spark} 3.x} \cite{spark_streaming} - framework wykorzystany do przetwarzania strumieniowego danych (\mbox{Spark Structured Streaming}) \cite{chambers2018spark} oraz do zadań uczenia maszynowego (\mbox{Spark ML}) w aplikacji \texttt{SparkDataProcessor},
    \item \textbf{framework \mbox{Spring Cloud}} - zestaw narzędzi do budowy aplikacji na chmurze,
    \item \textbf{biblioteka \texttt{React 18}} - biblioteka \texttt{JavaScript} wykorzystana do budowy interfejsu użytkownika.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{baza danych \texttt{PostgreSQL 15}} - relacyjna baza danych,
    \item \textbf{baza danych \texttt{Elasticsearch 8.7}} - baza danych \mbox{NoSQL} \cite{nosql_definition} do wyszukiwania i analizy,
    \item \textbf{baza danych \texttt{Redis 7.0}} - pamięciowa baza danych wykorzystywana jako rozproszona pamięć podręczna (ang. distributed cache).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{platforma \texttt{Docker}} - platforma konteneryzacji,
    \item \textbf{menedżer pakietów \texttt{Helm 3}} - menedżer pakietów dla \textbf{Kubernetes},
    ...
    \item \textbf{narzędzie \texttt{Terraform}} - narzędzie do zarządzania infrastrukturą jako kod (IaC, ang. Infrastructure as Code) \cite{terraform_docs}.
\end{itemize}

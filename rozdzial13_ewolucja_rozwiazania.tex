...
Alternatywną próbą było ręczne postawienie klastra na maszynach wirtualnych EC2 (ang. Amazon Elastic Compute Cloud). To podejście, choć tańsze, wiązało się z ogromną złożonością konfiguracyjną, długim czasem wdrażania i jest choć tańsze niż EKS, to jednak i tak generowało zbyt wiele kosztów. Tutaj wymagane jest wdrażanie całego klastra oraz aplikacji za pomocą skryptów AWS CloudFormation w celu zmniejszenia kosztów. AWS CloudFormation jest narzędziem AWS, które pozwala na deklaratywne definiowanie zasobów w postaci szablonów YAML. Jako zasoby można rozumieć tutaj instancje EC2, sieci VPC, konfiguracje zapory ogniowej, itp. Proces ten jednak trwa długo i jednorazowo zajmuje około 10 minut wraz z skryptami inicjalizującymi instancje oraz instalującymi wymagane aplikacje na systemach Linux. Rozwiązanie oparte na EC2 wymagało nadzoru od strony administratora klastra w celu redukcji kosztów lub automatyzacji włączania i wyłączania maszyn wirtualnych w zależności od potrzeb. Jednak klaster by pełnić swoją rolę nie może mieć przerw operacyjnych. Na listingu \ref{lst:eks_cloudformation} przedstawiono przykładowy skrypt AWS CloudFormation do wdrożenia klastra EKS. Oryginalny skrypt jest dużo bardziej skomplikowany, dlatego przedstawiono jego uproszczoną wersję.
...
Rozwiązanie oparte na EC2 wymagało zmienić. Koszty oraz elastyczność wdrażania zmian w klastrze wymagały poprawy. Dlatego zakupiono dwa komputery serwerowe fizyczne i zainstalowano na nich klaster ręcznie. Rozmiar wdrożenia i skala systemu na potrzeby tej pracy jest relatywnie mała (względem chociażby wdrożenia do prawdziwego zakładu przemysłowego).

Poniżej zostały przedstawione koszty w zależności od rozwiązania. Koszty obejmują tylko zasoby obliczeniowe, nie sieciowe. Koszty wyrażono w złotych polskich. Koszty dla zasobów AWS zostały oblczone za pomocą tej strony: \url{https://calculator.aws/}. Koszty związane z zasobami AWS są comiesięczne, a koszt zakupu infrastruktury lokalnej jest jednorazowy. Należy jednak zastrzec, że stosunki kosztów będą się różnić od skali wdrożenia i dla większych wdrożeń rozwiązania oparte na EC2/EKS mogą być niższe.
...
W pierwszej iteracji systemu do przetwarzania danych strumieniowych wykorzystano bibliotekę Kafka Streams. Zaletą tego rozwiązania była ścisła integracja z ekosystemem Apache Kafka oraz niskie opóźnienia dzięki przetwarzaniu rekord po rekordzie. Jednak w trakcie rozwoju pojawiły się następujące ograniczenia:
\begin{itemize}
    \item \textbf{Czytelność kodu} - Implementacja bardziej złożonej logiki biznesowej, zwłaszcza operacji stanowych i łączenia wielu strumieni, prowadziła do skomplikowanego i trudnego w utrzymaniu kodu. Kod zawierał dużą liczbę klas, które były trudne do zrozumienia i utrzymania.
...
Poniżej przedstawiono przykładowy kod implementacji w klasie \texttt{PressureMeanStreamFactory}.
...
Powyższy kod przedstawia klasę \texttt{PressureMeanStreamFactory}, która dziedziczy po generycznej klasie \texttt{MeanStreamFactory} i służy do przetwarzania strumieni danych ciśnienia za pomocą biblioteki Kafka Streams. Klasa ta implementuje logikę rozdzielania strumienia wejściowego na podstrumienie według typów urządzeń (sprężarka, turbina, kompresor, itp.), a następnie agreguje dane w oknach czasowych, licząc średnie wartości ciśnienia dla każdej etykiety. Wyniki są następnie publikowane na odpowiednie tematy wyjściowe.

\vspace{0.3em}

Kod ten jest jednak trudny do czytania i utrzymania z kilku powodów:
\begin{itemize}
...
W kodzie takich klas było kilka. Jego refaktoryzacja nie zwiększyła czytelności kodu, gdyż Kafka Streams operuje w większości na funkcjach anonimowych. Natomiast Apache Spark oferuje bardziej czytelny i zrozumiały interfejs API, co pozwoliło na zwiększenie czytelności kodu. Poniżej przedstawiono przykładowy kod implementacji w klasie \texttt{MeanProcessor}.

\begin{lstlisting}[caption={Implementacja w klasie MeanProcessor za pomocą Apache Spark}, label={lst:mean_processor},language=Scala]
...
Podjęto decyzję o przejściu na \textbf{Apache Spark Structured Streaming}. Mimo że działa on w modelu mikrowsadowym, co wprowadza nieco większe opóźnienia, przyniósł on kluczowe korzyści:
\begin{itemize}
    \item \textbf{wyższy poziom abstrakcji} - deklaratywne API Sparka, podobne do SQL, znacząco uprościło implementację logiki przetwarzania danych i poprawiło czytelność kodu,
...
Początkowy projekt relacyjnej bazy danych zakładał tworzenie osobnej tabeli dla każdego typu czujnika (np. temperatura, cisnienie, wibracje). Na listingu \ref{lst:mean_temperature} przedstawiono kilka tabel stosujących się do tego podejścia.

\begin{lstlisting}[caption={Przykładowe tabeli w relacyjnej bazie danych}, label={lst:mean_temperature},language=SQL]
...
Takie podejście, choć intuicyjne na początku, miało poważne wady:
\begin{itemize}
...
Zmieniono podejście na model danych opierający się na jednej tabeli, co przedstawiono na listingu \ref{lst:sensor_data}.

\begin{lstlisting}[caption={Przykładowa tabela w relacyjnej bazie danych}, label={lst:sensor_data},language=SQL]
...
Nowy, zunifikowany model danych opiera się na jednej, centralnej tabeli odczyty\_czujnikow, która zawiera kolumny takie jak typ\_czujnika, wartosc, jednostka itp. Ta zmiana przyniosła fundamentalne korzyści:
\begin{itemize}
    ...
    \item \textbf{uproszczenie zapytań} - analiza danych sprowadza się do prostych zapytań do jednej tabeli z odpowiednim filtrowaniem i grupowaniem.
\end{itemize}

\subsection{Zarządzanie wdrożeniami}

Pierwotnie stosowano ręczne zarządzanie manifestami YAML klastra Kubernetes. Każdy projekt aplikacji miał folder z własnymi manifestami. Skrypt ten, choć automatyzował część procesu, był proceduralny i wymagał implementacji logiki oczekiwania na gotowość poszczególnych zasobów (np. węzłów, usług, a nawet na konkretne zdarzenia z SNS). Na klastrze również istniała specjalna aplikacja, która zarządzała wdrażaniem nowych wersji aplikacji tzw. deployer. Narzędzie to nasłuchiwało kanał SNS. Deweloper musiał najpierw zaaktualizować repozytorium z aplikacją, później wysłać wiadomość na ten kanał SNS, następnie wspomniana aplikacja rozpoczyna proces wdrażania nowej wersji aplikacji. Cały proces pokazano na rysunku \ref{fig:deployer}.

\singlesizedimageforced{images/deployer-flow.png}{Proces wdrażania nowej wersji aplikacji}{1.0}

Jednak to rozwiązanie było mało elastyczne i aplikacja często się zawieszała. W celu poprawy stabilności, użyto menedżera pakietów \textbf{Helm}, który pozwala na deklaratywne definiowanie pożądanego stanu aplikacji w postaci szablonów (ang. charts). Następnie osadzono wszystkie szablony w jednym repozytorium, co pozwoliło na automatyczne wdrażanie nowych wersji aplikacji za pomocą jednej komendy. repozytorium służyło jako stan klastra Kubernetes.
...
Początkowo klaster Kubernetes był oparty na środowisku uruchomieniowym Docker. Jednakże, ze względu na zmiany w ekosystemie Kubernetes, podjęto decyzję o przejściu na \textbf{Containerd}. Jest to zgodne z obecnymi standardami i rekomendacjami dla klastra Kubernetes. Containerd, będący lżejszym i bardziej wyspecjalizowanym środowiskiem uruchomieniowym, oferuje lepszą wydajność i mniejszy narzut zasobów w porównaniu do pełnego silnika Dockera, co jest kluczowe w kontekście efektywnego zarządzania zasobami klastra.

\subsection{Architektura sieciowa}

Wczesna konfiguracja sieciowa, oparta na usługach typu NodePort i zewnętrznym AWS Application Load Balancer (ALB), była prosta w implementacji, ale miała istotne wady. Ruch musiał przechodzić przez dodatkową warstwę w chmurze AWS, co generowało duże opóźnienia. Co więcej, takie podejście utrudniało zarządzanie ruchem wewnątrz klastra i nie pozwalało na elastyczne wykorzystanie nazw DNS dla poszczególnych usług.

Zmienono sposób dostępu do aplikacji na klastrze. Użyto balansera obciążenia ruchu sieciowego \textbf{MetalLB} w trybie L2 do udostępniania zewnętrznych adresów IP w lokalnej sieci oraz kontroler \textbf{Nginx Ingress} do zaawansowanego routingu ruchu HTTP/HTTPS. Balanser \textbf{MetalLB} można zainstalować na klastrach Kubernetesa, które są osadzone na fizycznych maszynach. Pozwoliło to na możliwość lokalnego rozwiązywania nazw DNS dla poszczególnych usług oraz sprawiło, że odpowiedź z klastra była szybsza.

Ta kombinacja pozwoliła na:
\begin{itemize}
    \item \textbf{wyeliminowanie zależności od zewnętrznego load balancera chmurowego}, co obniżyło opóźnienia,

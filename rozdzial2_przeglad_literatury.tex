...
a warstwa przetwarzania opiera się na mechanizmach przetwarzania strumieniowego, takich jak Apache Kafka \cite{spark_documentation} czy Apache Flink \cite{flink}.
...
kontenerowymi \cite{kubernetes}. Powstała jako projekt Google, obecnie rozwijana przez Cloud Native Computing Foundation (CNCF) \cite{cncf_website}.
...
\subsubsection{Przedstawienie Apache Kafka}
...
Apache Software Foundation. Kafka oferuje następujące funkcjonalności:
...
\subsubsection{Przedstawienie Kafka Streams}
...
\subsubsection{Przedstawienie Apache Flink}
...
\subsubsection{Przedstawienie Apache Spark Streaming}
...
\subsubsection{Przedstawienie Apache Avro i Confluent Schema Registry}
...
Systemy SCADA (Supervisory Control and Data Acquisition) to tradycyjne rozwiązania do monitorowania i kontroli procesów przemysłowych \cite{scada}.
...
Istnieje szereg open source rozwiązań do analizy danych w czasie rzeczywistym, takich jak Apache NiFi \cite{apache_nifi},
